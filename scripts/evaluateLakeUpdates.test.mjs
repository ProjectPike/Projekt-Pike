...
test("real raw evaluator treats all applied proposals as stale history", async () => {
  const paths = [
  ...
  assert.equal(suite.summary.proposalCount, 15);
  assert.equal(suite.summary.eligibleProposalCount, 0);
  assert.equal(suite.summary.blockedProposalCount, 15);
  assert.equal(suite.summary.productionModified, false);
  assert.deepEqual(suite.results.filter(({ eligible }) => eligible), []);
